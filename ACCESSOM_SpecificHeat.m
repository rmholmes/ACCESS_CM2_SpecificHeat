...
plot_only = 1;
typ = 'month';
mname = ['ACCESSOM_SpecificHeat_' typ '.mat'];

if (~plot_only)

    base = '/scratch/e14/rmh561/access-om2/archive/025deg_jra55_iaf_cycle1/';
    name = 'ACCESSOM2_025';
    fname = [base 'output000/ocean/ocean_' typ '.nc'];
    gname = [base 'output000/ocean/ocean_grid.nc'];
...
CT = squeeze(ncread(fname,'sst',[1 1 1],[xL yL 1]));
mask = ~isnan(CT);
...
for fi = 1:length(outputs)

    fname = [base sprintf(['output%03d/ocean/ocean_' typ '.nc'],outputs(fi))];
        sprintf('Doing %03d of %03d',fi,length(outputs))
        ...
        PT = squeeze(ncread(fname,'sst',[1 1 1],[xL yL tL]));
        PS = squeeze(ncread(fname,'sss',[1 1 1],[xL yL tL]));
        if (max(max(max(PT)))>100); PT=PT-273.15; end;
        ...
        Q_t = ncread(fname,'net_sfc_heating') + ...
              ncread(fname,'frazil_3d_int_z');
...
% $$$         if (mod(fi,5)==0)
% $$$             save(mname,'OHC','time','DT_A','mask','Cp0','Cp0_teos10','CpR_mean', 'dCp_dS_mean','lon','lat','area', ...
% $$$                  'CpR_on_Cp0','Q','Qf','Q_ts','Qs','Qf_ts','Qs_ts','S','Sa','Qf_swp','Qf_swp_ts', ...
% $$$                  'PT_ts','PS_ts');%, ...
            save(mname,'time','DT_A','mask','Cp0','Cp0_teos10','CpR_mean','lon','lat','area', ...
                 'CpR_on_Cp0','Q','Qf','Q_ts','Qf_ts', ...
                 'PT_ts','PS_ts');%, ...
% $$$                  ,'Q_S','Qf_S');
% $$$         end
% $$$     end
